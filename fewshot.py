...
from datasets import ModelNet40Align, CO3D
from utils import IOStream


clip_model, _ = clip.load('ViT-B/32', device='cpu')
# co3d
data_train = CO3D('train')
data_test = CO3D('test')
...
prompts = data_train.get_label2name()
...
def train(args, io):
    train_dataloader = DataLoader(data_train, batch_size=args.batch_size, num_workers=4, shuffle=True)
    test_dataloader = DataLoader(data_test, batch_size=args.test_batch_size, num_workers=4, shuffle=True)
...
def eval(args):
    ...
    test_dataloader = DataLoader(CO3D('test'), batch_size=args.test_batch_size, num_workers=4, shuffle=True)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
