...
from models import FewShotCIL, focal_loss, FewShotCILwoRn, FewShotCILwoRn2,MYCIL
#################################################### TConfigurations ############################################
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
device_id = [0, 1]
exp_time = datetime.now().strftime("%Y_%m_%d_%H_%M_%S")
len_cls = [55, 59, 63, 67, 71, 75, 79] 
"""
Arguments
"""
...
parser.add_argument('-a', '--exp_name', type=str, default='shape2model_optimizer_reset', help='name of experiment')
...
def test_loop(dataloader, model, prompts_feats, loss_fn, stat,task_id):
    ...
    result = classification_report(ans_list, predict_list, target_names=id2name[:len_cls[task_id]])             
    print(result)
...
def train():
    ...
    # model = MYCIL(args,feats_p=feats_p.t(),use_dpa=True).to(device)
    ...
    for t in range(args.epoch0):
        ...
        train_loop(train_loader_0, model, prompts_feats[:num_cat_0], optimizer, loss_fn, runtime_stat)
    test_loop(test_loader_0, model, prompts_feats[:num_cat_0], loss_fn, runtime_stat,0)
    """
    # freeze params
    for name, param in model.named_parameters():
    ...
    for task_id in range(1, session_maker.tot_session()):
        ...
        test_loop(test_loader_i, model, prompts_feats[:num_cat_i], loss_fn, runtime_stat,task_id)
