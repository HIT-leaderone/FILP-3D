    ...
    if args.use_FSCIL3D_dataset:
        ...
        for points, label in tqdm(dataloader):
            ...
            if stat['task_id'] == 0: 
                logits = model(points, prompts_feats)
                loss_cont = 0  
            # if stat['task_id'] == 0: loss_cont = 0
            # else: loss_cont = model.calc_contloss(points, prompts_feats ,label, args.enhance_time)
            else: 
                logits = model(points, prompts_feats)
                args.enhance_time = 8
                loss_cont = model.calc_contloss(points, prompts_feats ,label, args.enhance_time)
